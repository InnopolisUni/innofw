...
for augmentation_type in ["augmentations_train", "augmentations_test", "augmentations_val"]:
    augmentation_configs_ttv.extend([[item] for item in (config_path / augmentation_type).rglob("*.yaml")])

# dataset_config_subdirs = []
for item in datasets_config_path.iterdir():
    if not any([i in str(item) for i in {"_infer", "tmqm", "qm9", "brain", "lung"}]) and not item.is_dir():
        datasets_config_files.append([item])
    # elif item.is_dir(): # some datasets are heavy, might exlude from testing
    #     dataset_config_subdirs.append(item)
...
def test_datasets(dataset_config_file, tmp_path):
    with open(dataset_config_file, "r") as f:
        ...
        for framework in Frameworks:
            try:
                dm = get_datamodule(dataset_config, framework, dataset_config['task'][0])
                break
            except Exception as e:
                logging.exception(e)

        assert dm is not None, f'Dataset {str(dataset_config_file)} failed to create'
...
@pytest.mark.parametrize(["augmentation_config", "key"], zip(augmentation_config_files, augmentation_keys))
def test_augmentations(augmentation_config, key):
    ...
    if augmentation_dictconfig is None or all([val is None for val in augmentation_dictconfig.values()]):
        return
    # print(augmentation_dictconfig)
    ...
    for i in range(max_iter):
        try:
            ...
            return
    # TODO: consider just forwarding the image
...
def test_augmentations_ttv(augmentation_config):
    # Reset global hydra state
    hydra.core.global_hydra.GlobalHydra.instance().clear()
    augmentation_config_rel = '/'.join(str(augmentation_config).split('/')[-2:])

    with initialize(version_base=None, config_path='../../../config'):
        augmentation_dictconfig = compose(config_name=augmentation_config_rel)
    
    # Empty config is skipped
    if augmentation_dictconfig is None or all([val is None or len(val) == 0 for val in augmentation_dictconfig.values()]):
        return
    augmentation = None
    for key in ["augmentations_train", "augmentations_val", "augmentations_test"]:
        try:
            augmentation = get_augmentations(augmentation_dictconfig[key]['augmentations'])
        except:
            continue
    assert augmentation is not None, f'Transform {augmentation_config} failed to create'

    import numpy as np
    image = np.random.random((224, 224, 3)).astype(np.float32)
    for transform in [lambda x: augmentation(image=x),
    ...
    assert image is not None, f'Transform {augmentation_config} failed'
