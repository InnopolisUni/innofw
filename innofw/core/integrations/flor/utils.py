...
def generate_text(task_prompt, tensor, onnx_file, tokenizer, text_input=None, dev="cpu"):
    ...
    decoder_input_ids = torch.tensor([[0]], device=dev)
...
def draw_bbox_on_image(image, results):
    # Извлекаем bounding box и метку из results
    ...
    # Цвет и ширина линии для прямоугольника
    bbox_color = (255, 0, 0)  # Красный цвет
    bbox_width = 3

    # Отрисовываем каждый bbox на изображении
    for bbox, label in zip(bboxes, labels):
        x_min, y_min, x_max, y_max = bbox  # Разворачиваем координаты bbox
        draw.rectangle(
            [x_min, y_min, x_max, y_max], outline=bbox_color, width=bbox_width
        )

        # Опционально: Отрисовываем текст метки рядом с bbox
        try:
...
def run_florence(task_prompt, tensor, size, text_input, tokenizer, ckpt_path:str, dev):
    # # Запускаем модель
    generated_text = generate_text(task_prompt, tensor, ckpt_path, tokenizer, text_input, dev)
    results = post_process_generation(generated_text, task_prompt, size, tokenizer)
    return results