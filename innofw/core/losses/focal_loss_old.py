...
class FocalLoss(_Loss):
    ...
    def _get_loss_value(self, output, target, weights):
        ...
        loss = F.nll_loss(logprobs, target.long(), reduction='none') # (N, H*W)
        ...
        if self.gamma == 0:
            ...
            return (weights * (1. - pt) ** self.gamma * loss).mean(dim=1)

    def forward(self, output, target):
        output = output.view(output.shape[0], -1)
        target = target.reshape(target.shape[0], -1)
        return self._get_loss_value(output, target, self._get_weights(target)).mean()