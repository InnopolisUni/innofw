...
class ImageToTextDatamodule(BaseLightningDataModule):
    ...
    def setup_train_test_val(self):
        ...
        if self.val_source:
            self.val_df = pd.read_csv(os.path.join(self.val_source, "captions.txt"))
    ...
    def val_dataloader(self):
        return DataLoader(
            ImageToTextDatamodule.ImageToTextDataset(
                self.val_source, self.val_df, self.word2int, self.max_caption_length
            )
        )
    ...
    def test_dataloader(self):
        return DataLoader(
            ImageToTextDatamodule.ImageToTextDataset(
                self.test_source, self.test_df, self.word2int, self.max_caption_length
            )
        )
    ...
    def train_dataloader(self):
        return DataLoader(
            ImageToTextDatamodule.ImageToTextDataset(
                self.train_source, self.train_df, self.word2int, self.max_caption_length
            )
        )
