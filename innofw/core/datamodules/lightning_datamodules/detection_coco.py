...
class DicomCocoDataModule(pl.LightningDataModule):
    def __init__(self, train,
                 test,
                 infer=None,
                 val_size: float = 0.2,
    ...
    def setup(self, stage=None):
        pass
        # # Создание датасета
        # full_dataset = DicomCocoDataset_sm(self.data_dir, transform=self.transform)
        #
    ...
    def predict_dataloader(self):
        # Создание датасета
        infer_ds = DicomCocoDataset_sm(data_dir = self.infer["target"], transform=self.transform)
        return DataLoader(infer_ds, batch_size=self.batch_size, shuffle=False, num_workers=4)
...
class DicomCocoDataModuleSegmentation(pl.LightningDataModule):
    def __init__(self, train,
                 test,
                 infer=None,
                 val_size: float = 0.2,
    ...
    def save_preds(self, preds, stage: Stages, dst_path: pathlib.Path):
        ...
        # Сохраняем все тензоры из списка как маски
        save_tensor_list_as_masks(preds, prefix="tensor_mask")




class DicomCocoDataModuleDetection(pl.LightningDataModule):
    def __init__(self, train,
                 test,
                 infer=None,
                 val_size: float = 0.2,
