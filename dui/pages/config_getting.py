    ...
    if exp_path.exists():
        with open(exp_path, "r") as yamlfile:
            try:
                ...
                print("ERROR:", exc)
    return (
        json.dumps({"success": True, "configuration_parameters": data}),
        200,
...
def parse_config_from_html(config_parameters, recurse=False):
    ...
    for el in config_parameters:
        if el["props"]["className"] == "uniter":
            ...
            k = el["props"]["children"][1]["props"]["children"]["props"][
                "value"
            ]
            v = (
                el["props"]["children"][2]["props"]["children"]["props"][
                    "value"
                ]
                if len(el["props"]["children"]) == 4
                else None
            )
            parent = k
            ...
            v = el["props"]["children"][2]["props"]["children"]["props"][
                "value"
            ]
            if not recurse:
                config_dict[k] = v
                ...
                configurations_html.append(
                    dbc.Row(
                        [
                            ...
                            dbc.Col(
                                dbc.Input(
                                    className="keyfield", value=k, type="text"
                                )
                            ),
                            dbc.Col(
                                dbc.Input(
