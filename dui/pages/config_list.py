...
dash.register_page(__name__, path="/config_list", title="config_list")
...
def layout():
    ...
    for p in experiment_configs_path.rglob("*"):
        if str(p).endswith(".yaml"):
            experiment_configs.append(
                str(p.relative_to(experiment_configs_path))
            )
    ...
    for i in range(0, 9999999999):
        exp_name = (
            f"New Experiment{i}.yaml" if i > 0 else "New Experiment.yaml"
        )
            ...
            dbc.Row(
                [
                    dbc.Col(
                        [
                            html.H4(
                                "Experiment Configurator", style={"height": 40}
                            )
                        ]
                    ),
                    dbc.Col(
                        ...
                        children=[
                            dcc.Link(
                                dbc.Button(
                                    ...
                                    id="create_exp",
                                    style={"width": 300, "margin-bottom": 5},
                                    n_clicks=0,
                                ),
                                href=f"/config/{exp_name}",
                                refresh=True,
                            ),
